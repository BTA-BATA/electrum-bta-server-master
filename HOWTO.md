...
The most up-to date version of this document is available at:
    ...
    https://github.com/pooler/electrum-bta-server/blob/master/HOWTO.md
...
document, we assume that user is called 'batacoin'). We also assume the
batacoin user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run batacoind and keep a copy of the full blockchain, 
which is roughly 4 GB in April 2014. If you have less than 2 GB of RAM 
make sure you limit batacoind to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running batacoind and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `batacoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser batacoin --disabled-password
    $ sudo apt-get install git
    $ sudo su - batacoin
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/batacoin/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download batacoind

We currently recommend batacoind 0.10.1.3 or 0.8.7.5 stable.

If you prefer to compile batacoind, here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config automake libtool
    $ sudo su - batacoin
    $ cd ~/src && git clone https://github.com/batacoin-project/batacoin.git -b master-0.10
    $ cd batacoin
    $ ./autogen.sh
    $ ./configure --disable-wallet --without-miniupnpc
    $ make
    $ strip src/batacoind src/batacoin-cli src/batacoin-tx
    $ cp -a src/batacoind src/batacoin-cli src/batacoin-tx ~/bin

### Step 3. Configure and start batacoind

In order to allow Electrum to "talk" to `batacoind`, we need to set up an RPC
username and password for `batacoind`. We will then start `batacoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.batacoin
    $ $EDITOR ~/.batacoin/batacoin.conf

Write this in `batacoin.conf`:
...
If you have an existing installation of batacoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ batacoind -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `batacoind`:

    $ batacoind

Allow some time to pass, so `batacoind` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ batacoin-cli getinfo

Before starting the electrum server your batacoind should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start batacoind at boot
time, running as the 'batacoin' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/pooler/electrum-bta-server.git
    $ cd electrum-bta-server
    $ sudo configure
...
and further instructions from the Electrum-bta full archival server foundry at:
http://foundry.electrum-bta.org/leveldb-dump/
...
Electrum reads a config file (/etc/electrum-bta.conf) when starting up. This
file includes the database setup, batacoind RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-bta.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-bta-server.
If you're using user batacoin to run electrum and have added it manually like shown in this HOWTO run 
the following code to add the limits to your /etc/security/limits.conf:

     echo "batacoin hard nofile 65536" >> /etc/security/limits.conf
     echo "batacoin soft nofile 65536" >> /etc/security/limits.conf

Two more things for you to consider:

1. To increase security you may want to close batacoind for incoming connections and connect outbound only

2. Consider restarting batacoind (together with electrum-bta-server) on a weekly basis to clear out unconfirmed
...
The magic moment has come: you can now start your Electrum server as root (it will su to your unprivileged user):
    ...
    # electrum-bta-server start

Note: If you want to run the server without installing it on your system, just run 'run_electrum_bta_server" as the
unprivileged user.
...
If you want to stop Electrum server, use the 'stop' command:
    ...
    # electrum-bta-server stop


If your system supports it, you may add electrum-bta-server to the /etc/init.d directory. 
...
safely whenever your machine is rebooted.
    ...
    # ln -s `which electrum-bta-server` /etc/init.d/electrum-bta-server
    # update-rc.d electrum-bta-server defaults
...
batacoins to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-bta and we'll try to congratulate you
on supporting the community by running an Electrum-bta node.

If you're operating a public Electrum-bta server please subscribe
to the following mailing list:
https://groups.google.com/forum/#!forum/electrum-bta-server
It'll contain announcements about important updates to Electrum-bta
