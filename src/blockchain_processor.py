...
class BlockchainProcessor(Processor):
    ...
    def do_catch_up(self):
        ...
        while not self.shared.stopped():
            ...
            if self.shared.paused():
                print_log("batacoind is responding")
                self.shared.unpause()
            time.sleep(10)
    ...
    def bitcoind(self, method, params=[]):
        ...
        while True:
            try:
                ...
                print_log("cannot reach batacoind...")
                self.wait_on_bitcoind()
            else:
                r = loads(respdata)
                if r['error'] is not None:
                    if r['error'].get('code') == -28:
                        print_log("batacoind still warming up...")
                        self.wait_on_bitcoind()
                        continue
    ...
    def getfullblock(self, block_hash):
        ...
        try:
            ...
            logger.error("batacoind error (getfullblock)",exc_info=True)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run batacoind with txindex=1; use -reindex if needed.")
